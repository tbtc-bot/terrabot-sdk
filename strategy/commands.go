...
func (sh *StrategyHandler) ParseCommandEvent(eventRaw []byte) {
	...
	// create new session
	session := terrabot.NewSession(event.BotId, event.UserId, event.AccessKey, event.SecretKey, event.PassPhrase, event.SimulationMode)
	session.Strategy.Symbol = event.Symbol
	session.Strategy.PositionSide = terrabot.PositionSideType(event.PositionSide)
